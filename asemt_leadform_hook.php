						 ...
						 foreach($response as $key){
										 ...
										 if($key['code'] == $parameters["google_key"]){
											   $pass = $key['code']; //checked in loop 115
												 ...
												 $sendto = '';
												 $emails = json_decode($key['email'], true);
												 if(is_array($emails)){
														for($i=0;$i<count($emails);$i++){

															$sendto = $sendto . ', ' . sanitize_email($emails[$i]);

														}
													}else{
														 $sendto = sanitize_email($key['email']);
													}
							 			...
							 			//  $body = "<h3>$subject</h3><br/>";
							 			    ...
							 			    $headers = array(
												...
												$mail = 0;
												 ...
												 if(!wp_mail( $contactEmail, $subject, $emailbody, $headers )){
													$mail = 1;
													}
												 if($mail == 0){
							 			        //$response['status'] = 200;
